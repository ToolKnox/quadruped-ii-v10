...
```python
rosrun robot_upstart install quadruped_peripheral_interfacing/launch/LCD_service.launch --job LCD_ros --symlink
```

In the line above, `quadruped_peripheral_interfacing` is the package, `LCD_service.launch` is the launch file and `LCD_ros`is the new job name that automates the launch file being called at startup. 
...
```python
rosrun quadruped_peripheral_interfacing quadruped_lcd_interfacing.py
```

The LCD module is currently set to run at startup using ROS Upstart, and from the quadruped.launch file.

# Create a pull request on Github

Include the Notion task link in your PR description. 

# Launch quadruped control in simulation

```python
source devel/setup.bash
roslaunch quadruped quadruped.launch is_physical:=0 is_sim:=1 launch_sim_environment:=1
```

# View joint or task space objectives

Make sure quadruped.launch has been started first then:
